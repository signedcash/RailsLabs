...
function ContainNodes(el, start)
{
	if (el.tagName === "HEAD" || el.tagName === "BODY" || el.tagName === "HTML")
	{
		...
		if (reg.test(el.tagName))
			tags.push(start + el.tagName);
	...
	else
	{
		if (el.nodeType === 3)
		{
			...
			if (reg.test(el.nodeValue))
				tags.push(start + el.nodeValue);
		}
		else
		{
			console.log(el.nodeName);
			if (reg.test(el.nodeName))
				tags.push(start + el.nodeName);
		}
